...
class GMC_Product_Fields {
    ...
    public function add_simple_product_fields() {
        ...
        echo '<h3 style="padding: 10px 12px; margin: 0; border-top: 1px solid #eee;">' . __('Multi-Currency Price Settings', 'multi-currency-woo') . '</h3>';

        // Base price field
        woocommerce_wp_text_input(array(
            'id' => '_gmc_base_price',
            'label' => __('Base Price', 'multi-currency-woo'),
            'desc_tip' => true,
            'description' => __('Enter the base price in the selected currency. This will be used to calculate the regular price.', 'multi-currency-woo'),
            'type' => 'number',
        ...
        woocommerce_wp_select(array(
            'id' => '_gmc_currency',
            'label' => __('Currency', 'multi-currency-woo'),
            'desc_tip' => true,
            'description' => __('Select the currency for the base price.', 'multi-currency-woo'),
    ...
    public function add_variation_fields($loop, $variation_data, $variation) {
        ...
        echo '<h4 style="margin: 0 0 10px 0;">' . __('Multi-Currency Settings', 'multi-currency-woo') . '</h4>';
        ...
        woocommerce_wp_text_input(array(
            ...
            'label' => __('Base Price', 'multi-currency-woo'),
            'desc_tip' => true,
            'description' => __('Enter the base price in the selected currency.', 'multi-currency-woo'),
            'type' => 'number',
            'custom_attributes' => array(
                'step' => '0.01',
                'min' => '0'
    ...
    public function add_manual_update_button() {
        ...
        <div class="options_group gmc-manual-update">
            <p class="form-field">
                <label><?php _e('Update Price Now', 'multi-currency-woo'); ?></label>
                <button type="button" class="button button-secondary gmc-manual-update-btn" data-product-id="<?php echo esc_attr($post->ID); ?>">
                    <?php _e('Update Regular Price from Base Price', 'multi-currency-woo'); ?>
                </button>
    ...
    public function add_variation_manual_update_button($loop, $variation_data, $variation) {
        ...
        <div class="gmc-variation-manual-update" style="padding: 10px 12px;">
            <button type="button" class="button button-secondary gmc-manual-update-btn" data-product-id="<?php echo esc_attr($variation_id); ?>" data-is-variation="1">
                <?php _e('Update Price from Base Price', 'multi-currency-woo'); ?>
            </button>
            <span class="gmc-manual-update-result"></span>
        </div>
        <?php
