...
class GMC_Price_Updater {
    ...
    public static function update_product_price($product_id, $dry_run = false) {
        ...
        if (!$product) {
            return array(
                'success' => false,
                'message' => __('Product not found', 'multi-currency-woo'),
        ...
        if (GMC_Exclusion_Manager::is_excluded($product_id)) {
            return array(
                'success' => false,
                'message' => __('Product is excluded from updates', 'multi-currency-woo'),
                'excluded' => true,
    ...
    private static function update_simple_product($product, $dry_run = false) {
        ...
        if (empty($base_price) || empty($currency_code)) {
            return array(
                'success' => false,
                'message' => __('No base price or currency set', 'multi-currency-woo'),
                'skipped' => true,
                'product_name' => $product->get_name(),
                'product_id' => $product_id
            );
        ...
        if (!$currency) {
            return array(
                'success' => false,
                'message' => sprintf(__('Currency %s not found', 'multi-currency-woo'), $currency_code),
                'product_name' => $product->get_name(),
                'product_id' => $product_id
            );
        }
    ...
    private static function update_variable_product($product, $dry_run = false) {
        ...
        if (empty($variation_ids)) {
            return array(
                'success' => false,
                'message' => __('No variations found', 'multi-currency-woo'),
                'skipped' => true,
                'product_name' => $product->get_name()
            );
        }
        ...
        if (!$has_updates) {
            return array(
                'success' => false,
                'message' => __('No variations with base price set', 'multi-currency-woo'),
                'skipped' => true,
                'product_name' => $product->get_name()
            );
        }
    ...
    private static function update_single_variation($variation_id, $dry_run = false) {
        ...
        if (!$variation) {
            return array(
                'success' => false,
                'message' => __('Variation not found', 'multi-currency-woo'),
