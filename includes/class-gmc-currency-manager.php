...
class GMC_Currency_Manager {
    ...
    private function add_currency() {
        ...
        if (empty($currency_code) || empty($currency_name) || $exchange_rate <= 0) {
            add_settings_error('gmc_messages', 'gmc_message', __('Please fill all required fields correctly.', 'woo-multi-currency-updater'), 'error');
            return;
        }
        ...
        if ($result) {
            add_settings_error('gmc_messages', 'gmc_message', __('Currency added successfully.', 'woo-multi-currency-updater'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', __('Failed to add currency. Currency code might already exist.', 'woo-multi-currency-updater'), 'error');
        }
    }
    ...
    private function edit_currency() {
        ...
        if ($result !== false) {
            add_settings_error('gmc_messages', 'gmc_message', __('Currency updated successfully.', 'woo-multi-currency-updater'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', __('Failed to update currency.', 'woo-multi-currency-updater'), 'error');
        }
    }
    ...
    private function delete_currency() {
        ...
        if (empty($id)) {
            add_settings_error('gmc_messages', 'gmc_message', __('Invalid currency ID.', 'woo-multi-currency-updater'), 'error');
            return;
        }
        ...
        if ($result) {
            add_settings_error('gmc_messages', 'gmc_message', __('Currency deleted successfully.', 'woo-multi-currency-updater'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', __('Failed to delete currency.', 'woo-multi-currency-updater'), 'error');
        }
