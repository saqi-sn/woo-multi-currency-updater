...
class GMC_Currency_Manager {
    ...
    private function add_currency() {
        ...
        if (empty($currency_code) || empty($currency_name) || $exchange_rate <= 0) {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Please fill all required fields correctly.', 'multi-currency-updater-for-woo'), 'error');
            return;
        }
        ...
        if ($result) {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Currency added successfully.', 'multi-currency-updater-for-woo'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Failed to add currency. Currency code might already exist.', 'multi-currency-updater-for-woo'), 'error');
        }
    }
    ...
    private function edit_currency() {
        ...
        if ($result !== false) {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Currency updated successfully.', 'multi-currency-updater-for-woo'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Failed to update currency.', 'multi-currency-updater-for-woo'), 'error');
        }
    }
    ...
    private function delete_currency() {
        ...
        if (empty($id)) {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Invalid currency ID.', 'multi-currency-updater-for-woo'), 'error');
            return;
        }
        ...
        if ($result) {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Currency deleted successfully.', 'multi-currency-updater-for-woo'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Failed to delete currency.', 'multi-currency-updater-for-woo'), 'error');
        }
