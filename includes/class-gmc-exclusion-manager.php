...
class GMC_Exclusion_Manager {
    ...
    private function remove_exclusion() {
        ...
        if (empty($product_id)) {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Invalid product ID.', 'multi-currency-updater-for-woo'), 'error');
            return;
        }
        ...
        if ($key !== false) {
            ...
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Product removed from exclusions.', 'multi-currency-updater-for-woo'), 'success');
        } else {
            add_settings_error('gmc_messages', 'gmc_message', esc_html__('Product not found in exclusions.', 'multi-currency-updater-for-woo'), 'error');
        }
